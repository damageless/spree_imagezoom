...
Deface::Override.new(:virtual_path => "spree/products/_thumbnails",
                      ...
                      :replace => "code[erb-loud]:contains('link_to')") do
                        "<div id='product_info_thumbnails'><a href=\"<%= @product.images.first.attachment.url(:large) %>\" rel=\"{gallery: 'product-gallery', smallimage: '<%= i.attachment.url(:large) %>', largeimage: '<%= i.attachment.url(:highres) %>'}\" class=\"<%= (i.id == @product.images.first.id) ? 'zoomThumbActive' : '' %> \" > <%= image_tag(i.attachment.url(:small), :class => 'product_info_thumbnail') %></a></div>"
                      end
